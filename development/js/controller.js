...
WechatBackupControllers.controller('TopBarController', ["$scope", "$rootScope", function ($scope, $rootScope) {
    ...
    $scope.exportLog = function () {
        ...
        bugout.log("system info: \n"+cp.execSync("sw_vers"))
        bugout.log("node version: "+cp.execSync("node -v", {encoding: "utf8"}))
        bugout.log("nwjs info: "+process.versions['node-webkit'])
        bugout.log("sqlite3 binding: "+cp.execSync("ls ./node_modules/sqlite3/lib/binding"))

        bugout.downloadLog()
        // $('#exportLogModal').modal('toggle')
    };
}]);
...
WechatBackupControllers.controller('NewEntryController', ["$scope", "$state", function ($scope, $state) {
    ...
    $scope.EntryController = function () {
        bugout.log('[In] New entry controller constructor');
        bugout.log('[Out] New entry controller constructor');
    };
